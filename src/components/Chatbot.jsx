...
const Message = ({ type, content }) => {
  ...
  return (
    <div
      className={`flex items-start ${isUser ? "justify-end" : "justify-start"} mb-4 animate-in fade-in duration-300 max-w-full`}
    >
      {!isUser && (
      ...
      <div
        className={`max-w-[80vw] sm:max-w-[70vw] md:max-w-[60vw] p-3 rounded-xl shadow-sm break-words ${
          isUser
...
const Chatbot = () => {
  ...
  return (
    <>
      {isChatbotOpen && (
        <div className="fixed inset-0 bg-black/30 z-[99] flex justify-end animate-in fade-in duration-300">
          <div
            className="relative w-full max-w-md md:w-96 h-full bg-white/95 backdrop-blur-lg rounded-l-2xl shadow-2xl shadow-emerald-500/20 flex flex-col animate-in slide-in-from-right-full duration-300 ease-out"
            role="dialog"
            aria-modal="true"
            aria-label="Avexra Help Bot"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-5 sm:p-6 border-b border-emerald-100 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-tl-2xl">
              <div className="flex items-center">
                <Robot size={28} className="mr-3 animate-bounce-slow" />
                <h2 className="text-lg sm:text-xl font-bold">Help Bot</h2>
              </div>
              <button
            ...
            <div className="flex-1 p-4 sm:p-6 overflow-y-auto custom-scrollbar">
              ...
              {isTyping && (
                <div className="flex items-start justify-start mb-4">
                  ...
                  <div className="max-w-[80vw] sm:max-w-[70vw] md:max-w-[60vw] p-3 rounded-xl bg-gray-100 text-gray-800 rounded-tl-none border border-gray-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 mb-4">
                {qaContent.map((q) => (
                  <button
                    ...
                    className="p-2 text-xs sm:text-sm bg-emerald-50/50 border border-emerald-200 rounded-full text-emerald-700 font-medium hover:bg-emerald-100 transition-colors duration-200 shadow-sm disabled:opacity-50"
                    disabled={isTyping}
                  >
                    {q.question}
                  </button>
